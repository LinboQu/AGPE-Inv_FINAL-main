...
def _preflight_cases(cases: list[str]) -> dict[str, tuple[dict, dict]]:
    ...
    if missing_lines:
        raise FileNotFoundError(
            "Missing required checkpoints/stats for paper ablation noise evaluation:\n"
            + "\n".join(missing_lines)
        )
    return case_cfgs
...
def run_paper_ablation_noise(cases: list[str], snrs: list[float], repeats: int, seed: int) -> None:
    results_root = Path("results")
    results_root.mkdir(parents=True, exist_ok=True)
    run_stamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    run_root = results_root / f"paper_ablation_noise_{run_stamp}"
    run_root.mkdir(parents=True, exist_ok=True)
    print(f"[RUN] paper ablation noise results root: {run_root.as_posix()}")
    ...
    detail_csv = run_root / "paper_ablation_noise_metrics_summary.csv"
    detail_xlsx = run_root / "paper_ablation_noise_metrics_summary.xlsx"
    ...
    delta_csv = run_root / "paper_ablation_noise_delta_summary.csv"
    delta_xlsx = run_root / "paper_ablation_noise_delta_summary.xlsx"
    ...
    if repeat_rows:
        repeat_csv = run_root / "paper_ablation_noise_repeatability_summary.csv"
        repeat_xlsx = run_root / "paper_ablation_noise_repeatability_summary.xlsx"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run AWGN noise evaluation for the five paper ablation cases."
    )
    parser.add_argument(
        "--cases",
        nargs="+",
        ...
        help="Paper ablation cases to evaluate.",
    )
    parser.add_argument(
        "--snrs",
        nargs="+",
        ...
        help="AWGN SNR levels in dB (default: 0 5 15 25 35).",
    )
    parser.add_argument(
        "--repeats",
        type=int,
